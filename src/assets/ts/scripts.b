...
dropdown?: MenuItem[];
    ...
    function updateActiveLinkOnScroll() {
        const headers = Array.from(document.querySelectorAll<HTMLElement>('.content h1, .content h2, .content h3, .content h4, .content h5'));
        const links = Array.from(document.querySelectorAll<HTMLAnchorElement>('#rightMenu .nav-link'));
        ...
        function setActiveLink() {
            ...
            if (activeIndex !== -1) {
                ...
                const currentLink = links.find(link => link.getAttribute('href') === `#${currentHeader.id}`);
                if (currentLink) {
                    links.forEach(link => link.classList.remove('active'));
